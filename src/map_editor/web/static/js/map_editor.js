...
// Build preview URL including dotted params when enabled
function previewUrl() {
...
function drawTempObject(obj) {
    ...
    if (obj.type === 'line') {
        ...
        ctx.beginPath(); ctx.arc(obj.x1, obj.y1, 4, 0, 2 * Math.PI); ctx.fill();
        ctx.beginPath(); ctx.arc(obj.x2, obj.y2, 4, 0, 2 * Math.PI); ctx.fill();
...
function setupEventListeners() {
    ...
    // Dotted view toggle and params
    const dottedCheckbox = document.getElementById('dotted-view');
    const dottedParams = document.getElementById('dotted-params');
    dottedCheckbox.addEventListener('change', (e) => {
        dottedView = e.target.checked;
        dottedParams.style.display = dottedView ? 'block' : 'none';
        loadMapPreview();
    });
    document.getElementById('dot-spacing').addEventListener('change', (e) => {
        dotSpacing = parseInt(e.target.value) || 15;
        if (dottedView) loadMapPreview();
    });
    document.getElementById('dot-radius').addEventListener('change', (e) => {
        dotRadius = parseInt(e.target.value) || 2;
        if (dottedView) loadMapPreview();
    });
    document.getElementById('dot-marker').addEventListener('change', (e) => {
        dotMarker = e.target.value;
        if (dottedView) loadMapPreview();
    });
    document.getElementById('dot-thin').addEventListener('change', (e) => {
        dotThin = e.target.checked;
        if (dottedView) loadMapPreview();
    });
...
function updateInstructions() {
    ...
    const messages = {
        ...
        'wall-line': 'Click start point, then end point — renders as dots in Dotted View',
