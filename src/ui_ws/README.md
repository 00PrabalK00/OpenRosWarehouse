...
Use your checkout root (for example `/home/next/testBuild`) as the canonical colcon workspace root.
This package supports a nested mirror at `src/` and uses symlinked package paths plus `src/COLCON_IGNORE` to avoid duplicate package discovery.
See `WORKSPACE_LAYOUT.md` for details.

## Source of Truth

`ugv_bringup` source is version-controlled under this package root:
...
`install/ugv_bringup/share/ugv_bringup` is a build artifact only. Do not treat it as editable source.

Validation command:

```bash
python3 scripts/verify_my_bot_source.py --source-root . --allow-missing-install
```

After build, this command will fail if source and install payloads drift.

## Workspace Mirror Sync

Use the mirror sync script to keep `src/ui_ws/src` aligned with top-level packages under `<workspace>/src`.

Top-level workspace build mode (recommended when building from `<workspace>`):

```bash
python3 scripts/sync_workspace_mirror.py --sync --check --colcon-ignore-mode top-level
```

Nested `ui_ws`-only build mode:

```bash
python3 scripts/sync_workspace_mirror.py --sync --check --colcon-ignore-mode nested
```

Consistency check only (non-mutating):

```bash
python3 scripts/sync_workspace_mirror.py --check --allow-missing-mirror
```

The CMake test target `my_bot_workspace_mirror` runs this check during `colcon test`.
...
- **High-Priority Stop Commands** - Published to `/wheel_controller/cmd_vel_unstamped` at 10 Hz to override navigation commands
...
- `/wheel_controller/cmd_vel_unstamped` - Manual + safety velocity commands (UI + teleop + safety)
...
├── ugv_bringup/              # Main robot package
│   ├── config/                # Configuration files
│   │   ├── nav2_params.yaml          # Navigation parameters (tuned for 20Hz)
│   │   ├── robot.yaml                # Operator-facing robot configuration
...
ros2 launch ugv_bringup launch_sim.launch.py use_sim_time:=true
```
...
```bash
source install/setup.bash
ros2 launch ugv_bringup zone_nav_ui.launch.py
```
...
Edit [unused/config/my_controllers.yaml](unused/config/my_controllers.yaml) to modify:
...
Edit [config/robot.yaml](config/robot.yaml) for joystick and operator input configuration.
...
- Verify velocity commands: `ros2 topic echo /wheel_controller/cmd_vel_unstamped`
...
- `/cmd_vel_joy` (Twist) - Joystick velocity commands (via twist_mux)
